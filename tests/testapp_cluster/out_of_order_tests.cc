        ...
        conn->mget(
                ...
                [&reorder, &counter]() -> FrameInfoVector {
                    ...
                    case Reorder::Half:
                        if (counter & 1) {
                            ret.emplace_back(
                                    std::make_unique<ReorderFrameInfo>());
                        }
                        break;
                    case Reorder::All:
                        ret.emplace_back(std::make_unique<ReorderFrameInfo>());
                        break;
                    }
                    ++counter;
                    return ret;
...
/**
 * Verify that if we send a pipeline with get commands without the Reorder bit
 ...
 */
TEST_F(OutOfOrderClusterTest, GetSequenceWithoutReorderBitSet) {
    testGetReorderWithAndWithoutReorder(Reorder::None);
}

/**
 * Verify that if we send a pipeline with get commands with the reorder bit
 ...
 */
TEST_F(OutOfOrderClusterTest, GetSequenceWithReorderBitSet) {
